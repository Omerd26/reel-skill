...
עדיין על המילים הנכונות — כולל זומים, אפקטים קוליים והוק).

**החלטות החיתוך מצטברות ונשמרות** ב-`work/cuts.json`: כל `--remove` נשאר עד שמבטלים אותו, וההגדרות
נשמרות לחיתוך הבא.
- `--remove 12.3-14.0` — חיתוך ידני (ציר המקור). `--restore 12.3-14.0` מחזיר אותו, `--restore all` את כולם.
- `--list` — כל החיתוכים (ידניים, מדרישות, שקטים) וההגדרות, בלי לחתוך.
- `--no-silence` / `--silence`, `--no-trim` / `--trim`, `--tail-pad 1.2` (אוויר אחרי המילה האחרונה —
  הגדל כשגרפיקה צריכה להישאר בסוף; ברירת מחדל 0.45), `--head-pad`.
- חיתוך שמבקשים בסרטון עצמו → `cut: true` בדרישה (מתבטל ב-`cut: false`).
...
5. **מיקום:** לכל אוברליי קבע `anchor` לפי הפנים בדפי הפריימים (טבלה ב-SCENES.md). חלק מהסוגים
   יושבים כברירת מחדל בגובה החזה — בסלפי עם פנים נמוכות זה נוחת על הפה. המאמת מזהיר.
6. **משהו שאין בקטלוג** (מספר ליד הדובר, לוגו שהמשתמש שלח, אנימציית פתיחה) → `custom_layers`
...
8. **מוזיקה:** `{"src": "music/<mood>/<file>.mp3", "volume": 0.10}` מתוך `<root>/remotion/public/music/`,
...
```
בוחר מסלול לבד (`--tier fast|full` לכפות), בודק שיש מקום בדיסק (~2.6MB לפריים), מאמת שוב, מרנדר
ל-`output/reel.mp4` באורך המדויק, מתקן את השהיית האודיו של המנוע, מגביל שיאי אודיו מעל ‎-1 dB,
ומריץ בדיקות איכות אוטומטיות. `caption_offset` (הזזת כתוביות בפיקסלים) עובד בשני המסלולים. נמדד: ~40ש' לקליפ של 9ש',
...
הדוח קשור לחתימת הקבצים שנבדקו (הסרטון, props, captions, requirements, החיתוך): כל שינוי אחרי
הבדיקה — גם תיקון קטן — פוסל אותו. אז: `render` (או `qa`) מחדש, צפייה מחדש, ו-`--finalize`.
